...
const AdminProducts = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await axios.get("http://192.168.0.101:8000/product");
        if (mounted) {
  ...
  const deleteProduct = async (id: string) => {
    ...
    try {
      await axios.delete(`http://192.168.0.101:8000/product/${id}`, {
  ...
  const toggleStatus = async (id: string, isActive: boolean) => {
    try {
      const url = `http://192.168.0.101:8000/product/${id}/${
